<?php
 ...
 $statement2 = $connect->prepare("
    SELECT sum(order_item_final_amount) as sum FROM tbl_order_item where created='$date2';
  ");
  $statement2->execute();
...
$statement3 = $connect->prepare("
    SELECT * FROM depenses where created='$date3';
  ");
  ...
  $totalrows = $statement3->rowCount();
   $statement4 = $connect->prepare("
    SELECT sum(montant) as cat FROM depenses where created='$date4'
  ");
  $statement4->execute();
...
<div id="html-2-pdfwrapper">
 ...
 <table class="table table-bordered">
          <div align="center">
    <input align="center" type="text" name="order_date" id="order_date" class="form-control input-sm" style="background-color:#9400D3; color: #FDFEFE; font-size:30px; text-align: center; width: 20%;" value="<?php echo $date; ?>"readonly />
         ...
         <tr>
            <td colspan="2">
                  ...
                  <br />
                  <table id="caisse-item-table" class="table table-bordered" style="width: 50%;">
                    <tr>
                    <th width="15%" style="border: solid; background-color:#FFB6C1; text-align: center;">Libellé</th>
                      <th width="15%" style="border: solid; background-color:#ADD8E6; text-align: center;">Quantité</th>
                      <th width="15%" style="border: solid;background-color:#FFFACD ; text-align: center;">Prix</th>
                      <th width="15%" style="border: solid;background-color:#FF0000; text-align: center;">Total</th>
                      <th width="15%" height="30px" style="border: solid;background-color:#90EE90; text-align: center;">Remarque</th>
                    </tr>
<?php
        if($total_rows > 0)
        {
          ...
          <td style="border: solid;"><input type="text" name="order_item_final_amount[]" id="order_item_final_amount'.$count.'" data-srno="'.$count.'" class="form-control input-sm  order_item_final_amount"  value="'.$row["order_item_final_amount"].'" readonly /></td>
           <td style="border: solid;"><textarea type="text" name="remarque[]" id="remarque'.$count.'" data-srno="'.$count.'" class="form-control input-sm  remarque" value style=" padding:0px; width:200px; height:30px"'.$row["remarque"].'">'.$row["remarque"].'</textarea></td>
       ...
       ?>
        ...
        <button type="button" name="add_row" id="add_row" class="btn btn-success btn-xs">+</button>
              ...
              <tr>
                ...
                <td  align="center" style="border: solid; font-size:20px;"><b>Total </td>
                <td  align="left" style="border: solid;font-size:20px; ;"><b><span  id="final_total_amt" ><?php echo number_format($sum_res['sum'] ); ?></span></b></td>
          ...
          <tr>
            <td align="center" style="border: solid; font-size:20px;" ><b>Balance</td>
          <tr>    
            <td align="left" style="border: solid; font-size:20px; background: #88f188; "><b><center><span  id="depenses_h2_id"><?php echo number_format($sum_res['sum']-$sum_rest['cat'], 3); ?></center></span></b></td>
            </tr>
            </tr>
            </table>
          </div>
...
if($all_results > 0)
        {
          ...
          foreach($all_results as $row)
          {
            ...
            echo '
            <tr id="row_id_'.$count.'">          
          <td style="border: solid;"><input type="text" name="ch_id[]" id="dep_id'.$count.'" value="'.$row["id"].'" hidden="true">
          <input type="text" name="nom[]" id="nom'.$count.'"  onChange="saveNewdep('.$count.')"  class="form-control input-sm" value="'.$row["nom"].'"/></td>
 ...
 ?>
  <!--<td><button type="hidden" name="removes_row" id="'.$count.'" class="btn btn-danger btn-xs removes_row">X</button></td> -->
              ...
              <table class="table table-bordered" style="width: 50%;">
              <tr>
                <td align="center" style="border: solid; font-size:20px; width:10%;"><b>Total Dépenses </td>
                <td align="center" style="border: solid; font-size:20px; width:10%;"><b><span id="final_amt" ><?php echo $sum_rest['cat'] ?></span></b></td>
              </tr>
              </table>
              <tr>
                <td colspan="2" align="center">
  ...
  var count = <?php echo $total_rows ?>;
        $('#total_item').val(count);
         ...
         $(document).on('click', '#add_row', function(){
          ...
          html_code += '<td style="border: solid;"><textarea type="text" name="remarque[]" id="remarque'+count+'" data-srno="'+count+'" class="form-control remarque" value="0"> </textarea></td>';
...
console.log(data)
 ...
 $('.remarque').change(function(){
  if($(this).val() != '')
  {
    let id = $(this).attr("data-srno");
    autosave(id);
  }
  }); 
...
function cal_diff(){
        ...
        var final_amt = $('#final_amt').text();
         ...
         $(document).on('click', '#adds_row', function(){
          ...
          var html_code = '';
          html_code += '<tr id="row_id_'+count+'">';          
          html_code += '<td style="border: solid;"><input type="text" name="nom[]" id="nom'+count+'" class="form-control input-sm" value="0" onChange="saveNewdep('+count+')" /></td>'; 
          html_code += '<td style="border: solid;"><input type="text" name="montant[]" id="montant'+count+'" data-srno="'+count+'" class="form-control input-sm montant" value="0" onChange="saveNewdep('+count+')" /></td>';
